...
class control_node():
    ...
    def __init__(self):
        ...
        self.im_breaking_init.data = 20.0
        self.yolo = 0
...
# Utility functions
    ...
    def find_breaking_coords(self,from_coords,to_coords):
        ...
        # lat_diff = abs(from_coords[0] - to_coords[0])*self.meter_conv[0]
        # long_diff = abs(from_coords[1] - to_coords[1])*self.meter_conv[1]
        # length = math.sqrt(lat_diff**2 + long_diff**2)
        # diff = [0, 0]
        #
        # diff[0] = 5*lat_diff/length
        # diff[1] = 5*long_diff/length
...
# Algorithm
    ...
    def control_flow(self):
        ...
        # Drone goes to above package
            ...
            elif (self.package_window_check([self.pickup_coordinate_list[self.current_package_index][0],
                                           self.pickup_coordinate_list[self.current_package_index][1], 0],
                                          [2, 2, float('inf')])):
                self.im_breaking_init.data = 25.7
                pass
        ...
        # Go above required return grid
            ...
            if (self.package_window_check([self.drop_coordinate_list[self.current_package_index][0],self.drop_coordinate_list[self.current_package_index][1], 0],[4, 4, float('inf')])):
                self.im_breaking_init.data = 25.7
                self.braking_pub_fun()
